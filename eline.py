...
class _E_LINE(object):

    def __init__(self, graph, rep_size=128, batch_size=10000, negative_ratio=5, order=3):
        self.cur_epoch = 0
        self.order = order
        self.g = graph
    ...
    def build_graph(self):
        ...
        self.h_e_context = tf.nn.embedding_lookup(
            self.context_embeddings, self.h)

        self.second_loss = -tf.reduce_mean(tf.log_sigmoid(
            self.sign * tf.reduce_sum(tf.multiply(self.h_e, self.t_e_context), axis=1))
            +tf.log_sigmoid(
                self.sign * tf.reduce_sum(tf.multiply(self.h_e_context, self.t_e), axis=1)))
...
class E_LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, epoch=10, negative_ratio=5, order=3):
        self.rep_size = rep_size
        self.order = order
        self.best_result = 0
        self.vectors = {}

        self.model = _E_LINE(graph, rep_size, batch_size,
